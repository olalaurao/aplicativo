...
Sync architecture (Google Drive, offline queue, conflict resolution via `_conflicts/`) is unchanged from V4 in shape. Its 3-way merge implementation had a confirmed, now-fixed defect — see the 2026-07 changelog entry above.

**Known issue, deliberately deferred (not yet a spec item):**
`SyncManager`'s conflict detection (`localChanged`/`remoteChanged` comparison against `baseHash`) is skipped entirely whenever no `baseHash` is recorded yet for a file — e.g. a fresh install, cleared app data, or a file's very first sync. In that state, two devices independently editing the same file can overwrite each other with no conflict raised (the fallback is a last-modified-timestamp heuristic, not a real 3-way check). Currently acceptable because Quartzo is single-account/single-primary-device in practice. **Must be revisited as part of public-release prep**, alongside the other single-vault/single-account architectural work already tracked there (OAuth/CASA audit, etc.) — before multi-device or multi-user usage becomes realistic.
