...
open class QuartzoCalendarWidgetProvider : AppWidgetProvider() {
    ...
    private fun buildViews(
        ...
        views.setOnClickPendingIntent(
            ...
            QuartzoWidgetUtils.openUriIntent(
                context,
                "Quartzo://widget-toggle?type=calendar_offset&offset=-1",
            )
        )
        views.setOnClickPendingIntent(
            R.id.nav_next,
    ...
    private fun applyDayWeek(
        ...
        if (mode == "day") {
            ...
            for (i in 0 until 7) {
                ...
                if (dayObj != null) {
                    ...
                    views.setTextViewText(dotsId, "â€¢".repeat(dotCount.coerceIn(1, 3)))
        ...
        val adapterIntent = Intent(context, QuartzoCalendarItemsService::class.java)
        adapterIntent.putExtra(AppWidgetManager.EXTRA_APPWIDGET_ID, appWidgetId)
        val adapterMode = data.optString("mode", mode)
        adapterIntent.data = Uri.parse("Quartzo://calendar-items/$appWidgetId/$adapterMode")
        views.setRemoteAdapter(R.id.items_list_view, adapterIntent)
        views.setPendingIntentTemplate(
            R.id.items_list_view,
            QuartzoWidgetUtils.backgroundTemplateIntent(context),
        )
    }
