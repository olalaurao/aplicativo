...
class _InsightsScreenState extends ConsumerState<InsightsScreen>
  ...
  Widget build(BuildContext context) {
    ...
    final blocksState = ref.watch(dashboardProvider('insights'));
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          IconButton(
                  icon: Icon(_editMode ? Icons.check_rounded : Icons.edit_rounded, color: AppTheme.accentColor(context)),
                  onPressed: () => setState(() => _editMode = !_editMode),
                  tooltip: _editMode ? 'Finish Editing' : 'Edit Insights Board',
                ),
        ],
      ),
      body: SafeArea(
            ...
            data: (blocks) {
              if (blocks.isEmpty) {
                ...
                WidgetsBinding.instance.addPostFrameCallback((_) async {
                  await ref.read(dashboardProvider('insights').notifier).addBlock(BlockType.timeBalance, 'Time Balance');
                  await ref.read(dashboardProvider('insights').notifier).addBlock(BlockType.whereTimeGoes, 'Where Time Goes');
                  await ref.read(dashboardProvider('insights').notifier).addBlock(BlockType.rhythmHeatmap, 'Rhythm Heatmap');
                  await ref.read(dashboardProvider('insights').notifier).addBlock(BlockType.energyChart, 'Energy Chart');
                  await ref.read(dashboardProvider('insights').notifier).addBlock(BlockType.rechargeVsDrain, 'Recharge vs Drain');
                  await ref.read(dashboardProvider('insights').notifier).addBlock(BlockType.focusByOrganizer, 'Focus by Organizer');
                  await ref.read(dashboardProvider('insights').notifier).addBlock(BlockType.plannedVsExecuted, 'Planned vs Executed');
                });
              ...
              if (_editMode) {
                ...
                return ReorderableListView.builder(
                  ...
                  ref.read(dashboardProvider('insights').notifier).reorderBlocks(oldIndex, newIndex);
                },
                  itemBuilder: (context, index) {
                    if (index == visibleBlocks.length) {
                      return Padding(
                        ...
                        child: InkWell(
                          onTap: () => AddComponentSheet.show(context, 'insights'),
                            ...
                            child: Row(
                              ...
                              children: [
                                IconButton(
                                  ...
                                  onPressed: () => ref.read(dashboardProvider('insights').notifier).toggleVisibility(block.id),
                                ),
                                const SizedBox(width: 4),
                                IconButton(
                                  icon: Icon(Icons.settings_rounded, color: AppTheme.textPrimaryColor(context)),
                                  style: IconButton.styleFrom(backgroundColor: AppTheme.surfaceColor(context)),
                                  onPressed: () => DashboardComponentConfigSheet.show(context, block, 'insights'),
                                ),
                                const SizedBox(width: 4),
                                IconButton(
                                  icon: const Icon(Icons.delete_rounded, color: AppColors.error),
                                  style: IconButton.styleFrom(backgroundColor: AppTheme.surfaceColor(context)),
                                     onPressed: () => ref.read(dashboardProvider('insights').notifier).removeBlock(block.id),
                                ),
                              ],
                            ),
                          ),
